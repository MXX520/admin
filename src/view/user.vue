    ...
    <section>
      ...
      >
        ...
        <el-table-column label="操作" width="360" align="center" fixed="right">
          <template scope="scope">
            <el-button size="small" @click="resetD(scope.$index, scope.row)"
              >重置</el-button
            >
            <el-button size="small" @click="handleD(scope.$index, scope.row)"
              >详情</el-button
            >
            <el-button size="small" @click="changeUser(scope.$index, scope.row)"
              >修改</el-button
            >
            <el-button
...
export default {
  ...
  methods: {
	...
	// -----
	async getSelectTop() {
      let { data } = await this.$api.get("role/list/all", {});
	  console.log("获取数据列表----",data)
      this.optionsTop = data.data;
    },
    async getSList() {
    ...
    async getUsers() {
      ...
      let para = {
		...
		type:this.valueTop ? this.valueTop : 0,
      };
		console.log(para);
